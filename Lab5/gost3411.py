...
def add_512(a, b):
    ...
    return ((x + y) & ((1 << 512) - 1)).to_bytes(64, 'little')

def add_int_512(a, val):
    x = int.from_bytes(a, 'little')
    return ((x + val) & ((1 << 512) - 1)).to_bytes(64, 'little')
...
def gost3411(data, hash_len = 512):
    if hash_len not in (256, 512):
        raise ValueError("hash length must be 256 or 512")
    ...
    while len(data) >= 64:
        ...
        h = g(N, m, h)
        N = add_int_512(N, 512)
        Sigma = add_512(Sigma, m)

    rem = data
    m = rem + b'\x01' + b'\x00' * (63 - len(rem))

    h = g(N, m, h)
    N = add_int_512(N, len(rem) * 8)
    Sigma = add_512(Sigma, m)
